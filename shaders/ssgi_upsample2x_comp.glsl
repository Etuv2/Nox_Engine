...
// Input: sampler2D inLow (quarter), sampler2D inHigh (half/working resolution - guidance), depthTex (full), normalTex (full)
...
if (any(greaterThanEqual(id, dstSize))) return;
    ...
    // CRITICAL FIX: Joint bilateral upsample with 3x3 kernel (matches SSAO logic)
  ...
  vec2 uvNWork = uvWork + vec2(x, y) * invDst;
    ...
    vec3 neighborColor = textureLod(inLow, uvNWork, 0).rgb;
       ...
       // CRITICAL FIX: Use exact same weight calculation as SSAO bilateral blur
...
float normalDot = max(dot(centerNormal, neighborNormal), 0.0);
 ...
 weightSum += weight;
    ...
    // Normalize result
    vec3 upsampled = (weightSum > 1e-6) ? (result / weightSum) : textureLod(inLow, uvWork, 0).rgb;

    // CRITICAL FIX: Blend with high-res guidance to preserve detail (subtle blend)

...
uniform int u_restirM;            // Initial candidates
uniform int   u_restirTemporalM;    // Temporal reuse count
uniform int   u_restirSpatialRadius;// Spatial reuse radius (reserved)
...
ReSTIRReservoir createEmptyReservoir() {
	ReSTIRReservoir r;
	r.lightIndex = -1;
	r.weightSum  = 0.0;
	r.targetPDF  = 0.0;
...
void updateReservoir(inout ReSTIRReservoir reservoir,
					 int lightIdx, vec3 lightPos,
					 vec3 radiance, float targetPDF, float weight) {
	reservoir.weightSum += weight;
	reservoir.M += 1;
	if (randomFloat() * reservoir.weightSum < weight) {
...
void combineReservoirs(inout ReSTIRReservoir r1, ReSTIRReservoir r2,
					   vec3 hitPos, vec3 normal, Material mat) {
	if (r2.lightIndex < 0 || r2.M == 0) return;

	vec3 toLight = r2.position - hitPos;
	float dist = length(toLight);
	vec3 lightDir = toLight / max(dist, EPSILON);

	if (dot(normal, lightDir) <= 0.0) return;

	float weight = r2.weightSum * float(r2.M);
	updateReservoir(r1, r2.lightIndex, r2.position, r2.radiance, r2.targetPDF, weight);
}

vec3 finalizeReservoir(ReSTIRReservoir reservoir,
					   vec3 hitPos, vec3 normal, vec3 viewDir, Material mat) {
	if (reservoir.lightIndex < 0 || reservoir.M == 0 || reservoir.weightSum <= 0.0) {
		return vec3(0.0);
	}

	vec3 toLight = reservoir.position - hitPos;
	float dist = length(toLight);
	vec3 lightDir = toLight / max(dist, EPSILON);

	vec3 brdf = evaluateBRDF(mat, normal, viewDir, lightDir);
	float w = reservoir.weightSum / (reservoir.targetPDF * float(reservoir.M) + 0.0001);
	return reservoir.radiance * brdf * w;
}

ReSTIRReservoir generateInitialReSTIRSamples(vec3 hitPos, vec3 normal, vec3 viewDir,
											 Material mat, int M) {
	ReSTIRReservoir reservoir = createEmptyReservoir();
	...
	for (int i = 0; i < M; ++i) {
		...
		float lightPDF;
		vec3 radiance = sampleLight(lightIdx, hitPos, lightDir, lightPDF);
		if (lightPDF <= 0.0 || dot(normal, lightDir) <= 0.0) continue;

		RTLightData light = lights[lightIdx];
		float maxDist = (int(light.position.w) == 0) ? 10000.0 : light.attenuation.w;
		if (!traceShadowRay(hitPos, lightDir, maxDist)) continue;

		vec3 brdf = evaluateBRDF(mat, normal, viewDir, lightDir);
		float targetPDF = dot(radiance * brdf, vec3(0.2126, 0.7152, 0.0722));

		float weight = targetPDF / (lightPDF + 0.0001);

		vec3 lightPos = (int(light.position.w) == 0)
					  ? (hitPos + lightDir * 1000.0)
					: light.position.xyz;

		updateReservoir(reservoir, lightIdx, lightPos, radiance, targetPDF, weight);
	}
...
void main() {
	...
	if (u_enableReSTIR && u_lightCount > 0) {
		int reservoirIdx = pixel.y * int(u_resolution.x) + pixel.x;

		ReSTIRReservoir reservoir = generateInitialReSTIRSamples(
			worldPos, normal, V, gbufferMat, u_restirM
		);

		if (u_frameIndex > 0 && u_restirTemporalM > 0) {
			ReSTIRReservoir prevReservoir = reservoirs[reservoirIdx];
			combineReservoirs(reservoir, prevReservoir, worldPos, normal, gbufferMat);
		}

		reservoirs[reservoirIdx] = reservoir;

		vec3 directLight = finalizeReservoir(reservoir, worldPos, normal, V, gbufferMat);
		color += directLight;

...
{
	// Reserve capacity for frame time data to avoid repeated allocations
...
void MainWindow::Render(float fps)
{
	...
	// Check if window is visible before doing expensive rendering
	...
	// Frame data changes every frame, so always update
	...
	// Only swap buffers if window is visible to avoid blocking on hidden/minimized windows
	if (isVisible) {

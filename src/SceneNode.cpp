    ...
    unsigned int defaultShaderProgram)
{
    ...
    auto bindTextureWithFallback = [](GLint uniformLoc, GLuint unit, GLuint texId, GLuint fallback) {
        glActiveTexture(GL_TEXTURE0 + unit);
        GLuint actualTexture = (texId != 0) ? texId : fallback;
        ...
        if (error != GL_NO_ERROR) {
            std::cerr << "[SceneNode] CRITICAL: OpenGL error " << error << " binding texture " << actualTexture << " to unit " << unit << std::endl;
        }
    };
...
void SceneNode::DrawGeometry(const glm::mat4& parentTransform, unsigned int geometryShader) {
    ...
    auto bindTextureIfValid = [](GLint uniformLoc, GLuint unit, GLuint texId) { if (texId != 0 && glIsTexture(texId)) { glActiveTexture(GL_TEXTURE0 + unit); glBindTexture(GL_TEXTURE_2D, texId); if (uniformLoc >= 0) glUniform1i(uniformLoc, unit); return true; } return false; };
    ...
    if (m_model) {
        for (auto& mesh : m_model->meshes) {
            ...
            ApplyCullingState(mesh, m_cullingOverride);
            if (mesh.RequiresAlphaBlending()) { glEnable(GL_BLEND); glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA); glDepthMask(GL_FALSE); } else { glDisable(GL_BLEND); glDepthMask(GL_TRUE); }
            if (mesh.RequiresAlphaTesting()) { if (GLint locAlphaCutoff = glGetUniformLocation(geometryShader, "alphaCutoff"); locAlphaCutoff >= 0) glUniform1f(locAlphaCutoff, mesh.alphaCutoff); if (GLint locUseAlphaTest = glGetUniformLocation(geometryShader, "useAlphaTest"); locUseAlphaTest >= 0) glUniform1i(locUseAlphaTest, 1);} else { if (GLint locUseAlphaTest = glGetUniformLocation(geometryShader, "useAlphaTest"); locUseAlphaTest >= 0) glUniform1i(locUseAlphaTest, 0);}            
            if (GLint locHasBaseColorTex = glGetUniformLocation(geometryShader, "hasBaseColorTexture"); locHasBaseColorTex >= 0) glUniform1i(locHasBaseColorTex, mesh.diffuseTexture != 0);
            if (GLint locHasNormalTex = glGetUniformLocation(geometryShader, "hasNormalTexture"); locHasNormalTex >= 0) glUniform1i(locHasNormalTex, mesh.normalTexture != 0);
            if (GLint locHasMR = glGetUniformLocation(geometryShader, "hasMetallicRoughnessTexture"); locHasMR >= 0) glUniform1i(locHasMR, mesh.roughnessTexture != 0);
            if (GLint locHasEmis = glGetUniformLocation(geometryShader, "hasEmissiveTexture"); locHasEmis >= 0) glUniform1i(locHasEmis, mesh.emissiveTexture != 0);
            if (GLint locHasOcc = glGetUniformLocation(geometryShader, "hasOcclusionTexture"); locHasOcc >= 0) glUniform1i(locHasOcc, mesh.occlusionTexture != 0);
            if (GLint locHasSpec = glGetUniformLocation(geometryShader, "hasSpecularTexture"); locHasSpec >= 0) glUniform1i(locHasSpec, mesh.specularTexture != 0);

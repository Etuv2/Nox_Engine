...
void InputManager::Update(float deltaTime) {
    ...
    // Clear frame-based states at the end of the frame
    ClearFrameStates();
}
...
void InputManager::HandleGamepadButtonEvent(const SDL_ControllerButtonEvent& buttonEvent) {
    int gamepadIndex = buttonEvent.which;
...
void InputManager::HandleGamepadAxisEvent(const SDL_ControllerAxisEvent& axisEvent) {
    int gamepadIndex = axisEvent.which;
    if (gamepadIndex < 0 || gamepadIndex >= static_cast<int>(m_gamepadStates.size())) return;
    if (!m_gamepadStates[gamepadIndex].connected) return;
...
void InputManager::HandleGamepadDeviceEvent(const SDL_ControllerDeviceEvent& deviceEvent) {
    switch (deviceEvent.type) {
        case SDL_CONTROLLERDEVICEADDED:
            std::cout << "[InputManager] Gamepad connected: " << deviceEvent.which << std::endl;
            InitializeGamepads(); // Reinitialize to pick up new gamepad
            break;
            
        case SDL_CONTROLLERDEVICEREMOVED:
            std::cout << "[InputManager] Gamepad disconnected: " << deviceEvent.which << std::endl;
            if (deviceEvent.which < static_cast<int>(m_gamepadStates.size())) {
                auto& gamepad = m_gamepadStates[deviceEvent.which];
        ...
        case SDL_CONTROLLERDEVICEREMAPPED:
            std::cout << "[InputManager] Gamepad remapped: " << deviceEvent.which << std::endl;
            break;
    }
}
...
void InputManager::InitializeGamepads() {
    ...
    std::cout << "[InputManager] Found " << numJoysticks << " joystick(s)" << std::endl;
    
    // Resize gamepad states array to accommodate all possible gamepads
    m_gamepadStates.resize(std::max(4, numJoysticks)); // Support at least 4 gamepads
    
    for (int i = 0; i < numJoysticks; ++i) {
        if (SDL_IsGameController(i)) {
            SDL_GameController* controller = SDL_GameControllerOpen(i);
            if (controller) {
                if (i < static_cast<int>(m_gamepadStates.size())) {
                    m_gamepadStates[i].controller = controller;
                    m_gamepadStates[i].connected = true;
                    
                    const char* name = SDL_GameControllerName(controller);
                    std::cout << "[InputManager] Initialized gamepad " << i << ": " 
                              << (name ? name : "Unknown") << std::endl;
                }
            } else {
                std::cerr << "[InputManager] Failed to open gamepad " << i << ": " 
                          << SDL_GetError() << std::endl;
            }
        }
    }

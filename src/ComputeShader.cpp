...
GLuint ComputeShader::CreateComputeTexture2D(GLuint width, GLuint height, 
                                            GLenum format, const void* data) {
    GLuint texture;
    glGenTextures(1, &texture);
    glBindTexture(GL_TEXTURE_2D, texture);
    
    // Determine the external format and type based on internal format
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, format, width, height, 0, externalFormat, dataType, data);
    ...
    std::cout << "[ComputeShader] Created compute texture: " << texture 

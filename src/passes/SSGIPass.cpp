...
// Helper to create floating-point 2D texture
...
void SSGIPass::allocTargets(int w, int h, bool halfRes) {
    ...
    createFloatTex2D(m_historyColor, w, h, GL_RGBA16F); // Previous frame color
    createFloatTex2D(m_historySSGI, w, h, GL_RGBA16F);  // Previous frame SSGI
}

void SSGIPass::freeTargets() {
    GLuint arr[] = { m_dirTex, m_ssgiRaw, m_ssgiQuarter, m_ssgiQuarterBlur, m_ssgiBlur, m_ssgiTex, m_historyColor, m_historySSGI };
    for (GLuint& t : arr) {
        if (t) { 
...
void SSGIPass::runDirections(RenderContext& ctx) {
    // Stage 1: Generate stochastic cosine-weighted directions per pixel
...
void SSGIPass::runRaymarch(RenderContext& ctx) {
    // Stage 2: Screen-space ray marching to find indirect lighting
...
void SSGIPass::runDownsample(RenderContext& ctx) {
    // Stage 2.5: Downsample half -> quarter
...
void SSGIPass::runBilateral(RenderContext& ctx) {
    // Stage 3: Edge-aware bilateral blur for denoising (quarter-res)
...
void SSGIPass::runUpsample(RenderContext& ctx) {
    // Stage 3.5: Bilateral upsample quarter->half and combine with raw half
...
void SSGIPass::runTemporal(RenderContext& ctx) {
    // Stage 4: Temporal accumulation for stability
    ...
    // Bind output texture with RGBA16F format
    glBindImageTexture(2, m_ssgiTex, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA16F);
    
    // Set temporal blend factor
    glUniform1f(glGetUniformLocation(m_csTemporal->GetProgramID(), "alpha"), 
                ctx.ssgiTemporalAlpha);
...
void SSGIPass::CaptureHistory(RenderContext& ctx) {
    // Capture current frame's final color as history for next frame's ray marching
    // This should be called AFTER PostProcess to get the final tonemapped result
    if (!ctx.hdrFBO) return;

    // Use glBlitFramebuffer to downsample/copy HDR color to history
    glBindFramebuffer(GL_READ_FRAMEBUFFER, ctx.hdrFBO->GetFBO());
    ...
    // Blit with linear filtering (downsamples if half-res)

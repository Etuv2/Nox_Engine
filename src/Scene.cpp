    ...
    bool hasAlpha)
{
    ...
    mesh.diffuseTexture = mesh.normalTexture =
        mesh.roughnessTexture = mesh.emissiveTexture = mesh.occlusionTexture = mesh.specularTexture = 0;
...
/**
 * Loads a glTF texture at texIndex, returning an OpenGL texture ID.
 * Returns 0 on failure.
 */
static GLuint LoadTextureFromGLTF(const tinygltf::Model& model, int texIndex)
{
    ...
    for (size_t mm = 0; mm < model.meshes.size(); mm++) {
        ...
        for (size_t p = 0; p < gltfMesh.primitives.size(); p++) {
            ...
            if (mesh.normalTexture == 0) {
                ...
                glGenTextures(1, &texID);
                glBindTexture(GL_TEXTURE_2D, texID);
                glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 1, 1, 0, GL_RGB, GL_UNSIGNED_BYTE, normalPixel);
                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
                mesh.normalTexture = texID;
            }
            if (mesh.roughnessTexture == 0) {
                ...
                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
                mesh.roughnessTexture = texID;
            }
            if (mesh.emissiveTexture == 0) {
                ...
                glGenTextures(1, &texID);
                glBindTexture(GL_TEXTURE_2D, texID);
                glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 1, 1, 0, GL_RGB, GL_UNSIGNED_BYTE, emissivePixel);
                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
                mesh.emissiveTexture = texID;
            }
            if (mesh.occlusionTexture == 0) {
            ...
            if (mesh.specularTexture == 0) {
                ...
                glGenTextures(1, &texID);
                glBindTexture(GL_TEXTURE_2D, texID);
                glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 1, 1, 0, GL_RGB, GL_UNSIGNED_BYTE, specPixel);
                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
                mesh.specularTexture = texID;
            }
...
void Scene::Draw() {
    ...
    for (auto& mesh : meshes) {
        ...
        // Bind textures to specific units for glTF PBR
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, mesh.diffuseTexture);              // Base color
        
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, mesh.normalTexture);               // Normal map
        
        glActiveTexture(GL_TEXTURE2);
        glBindTexture(GL_TEXTURE_2D, mesh.roughnessTexture);            // Metallic-Roughness
        
        glActiveTexture(GL_TEXTURE3);
        glBindTexture(GL_TEXTURE_2D, mesh.emissiveTexture);             // Emissive
        
        glActiveTexture(GL_TEXTURE4);
        glBindTexture(GL_TEXTURE_2D, mesh.occlusionTexture);            // Occlusion

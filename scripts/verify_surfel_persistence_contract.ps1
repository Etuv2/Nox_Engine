...
Assert-Contains "src/passes/SurfelGIPass.cpp" "RunPersistentStateUpdate\(ctx, \*renderSystem, camera\);\s*RebuildSpatialGrid\(ctx\.view\);\s*RunRecycleDecision\(ctx\);" "Recycle decisions must use a spatial grid rebuilt after transform-relative persistent state update."
Assert-Contains "src/passes/SurfelGIPass.cpp" "RunPersistentTileCoverage\(ctx\);\s*RunCoverageGapFill" "Persistent surfel coverage must be applied before spawn gap fill."
Assert-Contains "src/passes/SurfelGIPass.cpp" "RunCoverageGapFill\(ctx, \*renderSystem, camera, invViewProj, invView\);\s*RunRecycleDecision\(ctx\);\s*RebuildSpatialGrid" "Recycle must react after same-frame gap-fill demand before rebuilding the final integration grid."
Assert-Contains "src/passes/SurfelGIPass.cpp" "constexpr uint32_t kSpawnIterations = 1;" "Coverage gap fill must be a bounded single tile pass, not a repeated respawn loop."
...
Assert-Contains "src/passes/SurfelGIPass.cpp" "debugInstanceCount = std::clamp\(m_lastStats.liveCount \+ 4096u" "Surfel debug views must avoid full fixed-pool draws in normal debug modes."
